...
def calculate_coordinate_matches(x_pred: np.ndarray, y_pred: np.ndarray, csv_path: str = "data/xy_data.csv") -> Tuple[int, int, int, float, float, float]:
    try:
        import pandas as pd
        df_str = pd.read_csv(csv_path, dtype=str)
        df_str.columns = [c.strip().lower() for c in df_str.columns]
        x_str = df_str["x"].values
        y_str = df_str["y"].values
        
        def get_decimals(s):
            if "." in s:
                return len(s.split(".")[1])
            return 0
        x_prec = np.array([get_decimals(s) for s in x_str])
        y_prec = np.array([get_decimals(s) for s in y_str])
        
        x_val = df_str["x"].astype(float).values
        y_val = df_str["y"].astype(float).values
        
        x_pred_rounded = np.array([round(xp, p) for xp, p in zip(x_pred, x_prec)])
        y_pred_rounded = np.array([round(yp, p) for yp, p in zip(y_pred, y_prec)])
        
        x_matches = int(np.sum(x_pred_rounded == x_val))
        y_matches = int(np.sum(y_pred_rounded == y_val))
        xy_matches = int(np.sum((x_pred_rounded == x_val) & (y_pred_rounded == y_val)))
        
        x_l1 = float(np.mean(np.abs(x_pred - x_val)))
        y_l1 = float(np.mean(np.abs(y_pred - y_val)))
        xy_l1 = float(np.mean(np.abs(x_pred - x_val) + np.abs(y_pred - y_val)))
        
        return x_matches, y_matches, xy_matches, x_l1, y_l1, xy_l1
    except Exception as e:
        logger.warning(f"Could not calculate coordinate matches: {e}")
...
def run_full_pipeline(x: np.ndarray, y: np.ndarray) -> FitResult:
    ...
    l1_residual = float(np.mean(np.abs(resid)))
